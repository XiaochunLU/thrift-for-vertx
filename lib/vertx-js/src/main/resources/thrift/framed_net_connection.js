...
var FramedNetConnection = exports.FramedNetConnection = function(port, arg1, arg2) {
  var self = this,
      host, options;
  ...
  this.transport = /*this.options.transport ||*/ ttransport.TFramedNetTransport;
  ...
  function connectHandler(jsocket) {
    var support = self.support = new ttransport.TFramedNetTransportSupport(jsocket, handler);
  ...
  var jclient = this.jclient = __jvertx.createNetClient();
  ssl_support.configureNativeClientSSL(jclient, options);
  if (options.reconnect_attempts)
    jclient.setReconnectAttempts(options.reconnect_attempts);
  if (options.reconnect_interval > -1)
    jclient.setReconnectInterval(options.reconnect_interval);
  if (options.connect_timeout)
    jclient.setConnectTimeout(options.connect_timeout);
  jclient.connect(port, host, helpers.adaptAsyncResultHandler(function(err, jsocket) {
    var addr = host + ':' + port;
    if (err) {
      console.error('thrift: Failed to connect to ' + addr);
    } else {
      console.log('thrift: Connected to ' + addr);
      connectHandler(jsocket);
    }
  }));
};

FramedNetConnection.prototype.end = function() {
  if (this.jclient) {
    this.jclient.close();
    this.jclient = null;
    this.support = null;
...
FramedNetConnection.prototype.write = function(data) {
...
exports.createFramedNetConnection = function(port, host, options) {
  return new FramedNetConnection(port, host, options);
};
